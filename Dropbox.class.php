...
class Dropbox
{
	...
	private static $CALLBACK_URL 		= 'http://localhost/Dropbox-master/Dropbox-PHP-API-master/index.php';
	...
	{
		return $this->call('https://api.dropbox.com/1/metadata/'.$root.'/'.$path);
	}

	function shares($root,$path)
	{
		return $this->call('https://api.dropbox.com/1/shares/'.$root.'/'.$path);
	}
	function media($root,$path)
	{
		return $this->call('https://api.dropbox.com/1/media/'.$root.'/'.$path);
	}
